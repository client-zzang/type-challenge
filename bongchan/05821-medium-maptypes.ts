...
/*
  🤔 접근
    ...
    2. Mapped Types를 활용한 제네릭 R의 mapFrom, mapTo 속성 활용하기

      type MapTypes<T, R extends { mapFrom: unknown; mapTo: unknown }> = {
        [P in keyof T]: T[P] extends R['mapFrom'] ? R['mapTo'] : T[P]; // ➡️ 값이 mapFrom으로 좁혀지는 경우에는 mapTo 활용
      };

      - ❌ 마지막 케이스(제네릭 R이 유니온)에서 에러 발생
